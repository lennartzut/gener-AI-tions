...
class IndividualService:
    ...
    Optional[Individual]:
        ...
        try:
            ValidationUtils.validate_date_order(
                individual_create.birth_date,
                individual_create.death_date,
            ...
            self.db.rollback()
            if "some_db_constraint" in str(e).lower():
                raise ValueError(
                    "Birth date must be before death date.")
            return None
    ...
    def get_individual_by_id(self, individual_id: int, user_id: int,
        ...
        try:
            ...
            if not individual:
                logger.warning(
                    f"Individual not found: ID={individual_id}")
            return individual
        except SQLAlchemyError as e:
            logger.error(
                f"Error retrieving individual by ID {individual_id}: {e}")
            return None
    ...
    def get_individuals_by_project(self, user_id: int,
        ...
        try:
            ...
            logger.info(
                f"Retrieved {len(individuals)} individuals for project {project_id}")
            return individuals
        except SQLAlchemyError as e:
            logger.error(
                f"Error retrieving individuals for project {project_id}: {e}")
            return []
    ...
    Optional[Individual]:
        ...
        try:
            ...
            if not individual:
                logger.warning(
                    f"Individual not found for update: ID={individual_id}")
                return None
            ...
            if primary_identity:
                identity_updates = {key: value for key, value in
                                    updates.items() if
                                    key in {"first_name",
                                            "last_name", "gender"}}
    ...
    def delete_individual(self, individual_id: int, user_id: int,
        ...
        try:
            ...
            if not individual:
                logger.warning(
                    f"Individual not found for deletion: ID={individual_id}")
                return False
