...
class RelationshipService:
    ...
    def create_relationship(self,
                            ...
                            project_id: int) -> Optional[Relationship]:
        """
        Creates a new relationship between two individuals within a project,
        ensuring a single canonical row. For parent-child, we store 'parent'
        as the initial_relationship. For partner, we store 'partner' once
        ...
        try:
            ...
            if individual_id == related_id:
                raise ValueError("Cannot create a self-relationship.")
            ...
            logger.info(f"Created canonical relationship: ID={new_rel.id}")
            return new_rel

        except SQLAlchemyError as e:
            self.db.rollback()
            logger.error(f"Database error creating relationship: {e}")
            if "chk_relationship_dates" in str(e).lower():
                raise ValueError("Union date must be before dissolution date.")
            return None
    ...
    def update_relationship(self,
        ...
        if initial_relationship changes from child to parent or vice versa.
        For partners, we also unify so only one row remains.
        """
        try:
            ...
            original_ids = (relationship.individual_id, relationship.related_id)
            original_type = relationship.initial_relationship

            updates = relationship_update.model_dump(exclude_unset=True, exclude={"relationship_detail"})
            ...
            if relationship_update.initial_relationship:
                ...
                elif relationship.initial_relationship == InitialRelationshipEnum.PARENT:
                    pass
            ...
            if existing and existing.id != relationship_id:
                raise ValueError("This relationship already exists with the new parameters.")
    ...
    def list_relationships(self, project_id: int) -> List[Relationship]:
        try:
            rels = self.db.query(Relationship).filter(Relationship.project_id == project_id).all()
