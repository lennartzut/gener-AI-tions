...
class IdentityService:
    ...
    def create_identity(self, identity_create: IdentityCreate, is_primary: bool = False) -> Optional[Identity]:
        """
        ...
        try:
            if is_primary:
                self.unset_primary_identity(identity_create.individual_id)
            ...
            new_identity = Identity(
                individual_id=identity_create.individual_id,
                first_name=identity_create.first_name,
                last_name=identity_create.last_name,
                gender=identity_create.gender,
                valid_from=identity_create.valid_from,
                valid_until=identity_create.valid_until,
            ...
            self.db.rollback()
            logger.error(f"Database error while creating identity: {e}")
            return None

    def get_identity_by_id(self, identity_id: int) -> Optional[Identity]:
        """
        ...
        try:
            ...
            if identity:
                ...
                logger.warning(f"Identity not found: ID={identity_id}")
            return identity
        except SQLAlchemyError as e:
            logger.error(f"Database error while retrieving identity: {e}")
            return None

    def update_identity(self, identity_id: int, identity_update: IdentityUpdate) -> Optional[Identity]:
        """
        ...
        try:
            ...
            self.db.rollback()
            logger.error(f"Database error while updating identity: {e}")
            return None
    ...
    def delete_identity(self, identity_id: int) -> bool:
        ...
        try:
            identity = self.get_identity_by_id(identity_id)
            if not identity:
                logger.warning(f"Identity not found for deletion: ID={identity_id}")
                return False
            ...
            self.db.rollback()
            logger.error(f"Database error while deleting identity: {e}")
            return False
    ...
    def unset_primary_identity(self, individual_id: int):
        ...
        try:
            ...
            if primary_identity:
                ...
                self.db.commit()
                logger.info(f"Unset primary identity: ID={primary_identity.id}")
        except SQLAlchemyError as e:
            self.db.rollback()
            logger.error(f"Database error while unsetting primary identity: {e}")
