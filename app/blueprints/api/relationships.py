...
def list_relationships():
    """
    List all relationships for a given project_id (short info).
    """
    user_id = validate_token_and_get_user()
    project_id = request.args.get("project_id", type=int)
    if not project_id:
    ...
    with SessionLocal() as session:
        ...
        relationships_out = [_short_relationship_dict(rel) for rel
                             in
                relationships]
...
@api_relationships_bp.route("/<int:relationship_id>", methods=["GET"])
@jwt_required()
def get_relationship(relationship_id):
    """
    Get a single relationship by ID (short info).
    """
    user_id = validate_token_and_get_user()
    project_id = request.args.get("project_id", type=int)
    if not project_id:
...
@api_relationships_bp.route("/<int:relationship_id>", methods=["PATCH"])
@jwt_required()
def update_relationship(relationship_id):
    ...
    get_project_or_404(user_id, project_id)
...
def _short_relationship_dict(rel: "Relationship"):
    """
    Return a minimal dictionary for a Relationship:
      - id, initial_relationship, notes, union_date, union_place, dissolution_date
      - individual's short info
      - related's short info
    """
    return {
        "id": rel.id,
